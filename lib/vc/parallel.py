...
def makeTaskAllSlices2p0test(tasks,id_dir,subDir,nTIsToFit,tiVec,maskMat,saveDir,M0=1,alpha=1,verbosity=1,dryRun=0,nBins=8,mMethod=0):
    iSlice=0;nSlices=14
    if verbosity>=2:
    ...
    while iSlice<nSlices:
        fitMask=np.zeros(np.shape(maskMat))
        fitMask[:,:,iSlice]=maskMat[:,:,iSlice]
        tasks.append( (iSlice, id_dir, subDir,fitMask,saveDir,nTIsToFit,tiVec,M0,alpha,verbosity,dryRun,nBins,mMethod ) )
